...
router.post('/chat', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const messages = [
            {
                role: 'system',
                content: 'أنت مساعد حجوزات لتطبيق Smart Maid لخدمات العاملات المنزلية. ' +
...
router.get('/estimate', auth_1.requireAuth, (req, res) => {
...
router.get('/context', auth_1.requireAuth, (req, res) => {
    const userBookings = store_1.bookings.filter((b) => b.userId === req.user.id);
    res.json({
